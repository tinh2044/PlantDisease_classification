...
def get_dataset(root_dir, size, batch_size):
    ...
    val_ds = val_ds.map(lambda x, y: (augmentation(x, training=True), y))
    train_ds = train_ds.cache().shuffle(len(train_ds))
    val_ds = val_ds.cache().shuffle(len(train_ds))
...
def train_disease_classification(root_dir, size, batch_size, epoch, export_dir=None, h5_dir=None):
    """
    ...
    for name in list_dataset_name:
        ...
        train_ds, val_ds, test_ds = get_dataset(f"{root_dir}/{name}", (size, size), batch_size)

        class_names = train_ds.class_names
        
        print(f"Get model for {name} ds")
        model = get_model((size, size, 3), len(class_names), model_name)
